    ...
    switch (inputType) {
      ...
      case "password":
        ...
        if (actionValue.value.length < 8) {
          passwordStateArray.push("- Au moins 8 caractères.");
        }
        if (!actionValue.value.match(lowerCaseLetters)) {
          passwordStateArray.push("- Au moins 1 minuscule.");
        }
        if (!actionValue.value.match(upperCaseLetters)) {
          passwordStateArray.push("- Au moins 1 majuscule.");
        }
        if (!actionValue.value.match(numbers)) {
          passwordStateArray.push("- Au moins 1 chiffre.");
        }
        if (!actionValue.value.match(specialChars)) {
          passwordStateArray.push("- Au moins 1 caractère spécial.");
        }
